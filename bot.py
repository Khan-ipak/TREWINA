...
def choose_count(call):
    ...
    if call.data == 'increment':
        bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.message_id,
                                      reply_markup=buttons.choose_amount(database.get_exact_pr
                                         (users[user_id]['pr_name'])[4], 'increment',
                                         users[user_id]['pr_count']))
        users[user_id]['pr_count'] += 1
    elif call.data == 'decrement':
        bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.message_id,
                                      reply_markup=buttons.choose_amount(database.get_exact_pr
                                                                         (users[user_id]['pr_name'])[4], 'decrement',
                                                                         users[user_id]['pr_count']))
        users[user_id]['pr_count'] -= 1
...
def get_location(message, text):
    ...
    if message.location:
        ...
        bot.send_message(762087553, text)
        bot.send_location(762087553, latitude=message.location.latitude, longitude=message.location.longitude)
        ...
        # Возрващение на этап получения локации
